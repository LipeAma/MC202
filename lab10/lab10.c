...
int main(void) {
  while (1) {
    int no_elements;
    if (scanf("%d", &no_elements) == EOF || no_elements == 0)
      break;
    ...
    while (q_size != 0) {
      ...
      if (pos > a) {
        ...
        if (insertPos >= no_elements)
          insertPos -= no_elements;
        queue[insertPos][0] = a;
      ...
      if (q_first == next_lvl_pos) {
        ...
        if (next_lvl_pos >= no_elements)
          next_lvl_pos -= no_elements;
      }
    }
    printf("\n");
  }
