...
int main(void) {
  ...
  while (scanf("%d %f %f %f %f", &n, &xc, &yc, &xr, &yr) == 5) {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      // Se o coelho já escapou, o loop só consumirá o stdin e não
      // realizará as outras operações. A condicional abaixo garante isso.
      if (escapou) continue;

      dc = (x - xc) * (x - xc) + (y - yc) * (y - yc);
      dr = (x - xr) * (x - xr) + (y - yr) * (y - yr);

      // O coelho chega primeiro se sua distancia for metade da distancia
      // da raposa. Isso acontece se o quadrado da sua distancia for
      // quatro vezes menor que o quadrado da distancia da raposa. A
      // condicional abaixo verifica isso.
      if (4 * dc < dr) {
    ...
    if (escapou == 0) printf("O coelho nao pode escapar.\n");
  }
}
